...
experiment_name = 'acrobot_mountain_to_cartpole'
...
class PolicyNetwork:
    def __init__(self, _state_size, _action_size, _learning_rate, name='policy_network'):
        ...
        with tf.variable_scope(name):
            ...
            self.mounatin_1 = tf.placeholder(tf.float32, [None, 12], name="state")
            ...
            self.Z1 = tf.add(self.Z1, self.mounatin_1)
...
class MountainPolicy:
    def __init__(self, _state_size, _action_size, _learning_rate, name='mountain_policy'):
        self.state_size = _state_size
        self.action_size = _action_size
        self.learning_rate = _learning_rate
...
def train(policy, value,mountain_policy,acrobot_policy):
    # Start training the agent with REINFORCE algorithm
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        mountain_res = tf.train.Saver({'policy_network/W1': mountain_policy.W1, 'policy_network/W2': mountain_policy.W2,
                                      'policy_network/b1': mountain_policy.b1, 'policy_network/b2': mountain_policy.b2})
        mountain_res.restore(sess, 'results/mountain_model/'+"model.ckpt")
        ...
        for episode in range(max_episodes):
            ...
            for step in range(max_steps):
                mountainA1 = sess.run(mountain_policy.A1, {mountain_policy.state:state})
                acrobatA1 = sess.run(acrobot_policy.A1, {acrobot_policy.state:state})
                actions_distribution, value_state = sess.run([policy.actions_distribution, value.estimated_value],
                                                             {policy.state: state, value.state: state, policy.acrobot_1: acrobatA1, policy.mounatin_1: mountainA1})
                ...
                feed_dict_policy = {policy.state: state, policy.R_t: lambda_value, policy.action: action_one_hot, policy.acrobot_1:acrobatA1, policy.mounatin_1:mountainA1}
                ...
                if done:
                    ...
                    if episode > 80 and average_rewards > 475:
...
def main():
    ...
    while not goal_reached:
        ...
        mountain_policy = MountainPolicy(state_size, action_size, policy_learning_rate)
        acrobot_policy = AcrobotPolicy(state_size, action_size, policy_learning_rate)

        goal_reached = train(policy, value,mountain_policy,acrobot_policy)
